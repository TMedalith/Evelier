...
function ProductCard({ product, index }) {
  return (
    <div key={index} className='min-w-max flex flex-col'>
      ...
      <a className="text-center font-archivoNarrow uppercase cursor-pointer font text-sm md:text-lg">{product.name}</a>
      <div className="text-center font-archivoNarrow text-lg  md:text-2xl font-semibold text-gray-700">${product.price}</div>
    </div>
  );
}
